...
ROSTER_FILE_PATH = os.getenv("ROSTER_FILE_PATH", os.path.join(os.path.dirname(__file__), "member_roster.txt"))
TASK_LOCK_FILE = os.getenv("TASK_LOCK_FILE", "/tmp/table_signup_background.lock")
BOUNDARY_REPORT_STATE_FILE = os.getenv("BOUNDARY_REPORT_STATE_FILE", "/tmp/table_signup_boundary_report.state")
...
def _get_activity_signups(activity_id, role=None):
    matches = []
    for signup in _list_signups():
        if str(signup.get(SIGNUP_COL_ACTIVITY_ID, '')).strip() != str(activity_id):
            continue
        if role and str(signup.get(SIGNUP_COL_ROLE, '')).strip() != role:
            continue
        matches.append(signup)
    return matches
